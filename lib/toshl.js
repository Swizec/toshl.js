...
exports.Toshl.prototype.expenses = function (params, to, callback) {
    ...
    if (params) {
        if (typeof params == 'number') {
            ...
            var from = params instanceof Date ? params : moment(params);
            to = to instanceof Date ? to : moment(to);
            
            options['from'] = from.toISOString();
            options['to'] = to.toISOString();
        }else if (params instanceof Array) {
            var tags = util.transform_tags(params);
            options[tags.type] = tags.tags;
        }
